...
const DEFAULT_HEADERS = {
  ...
  'user-agent': 'RoJoiner/1.2 (+public-presence-only)',
};
...
export async function searchPublicPresence(rawUsername) {
  ...
  const [game] = await Promise.all([getGameDetails(placeId)]);
  ...
  if (exactServerAvailable) {
    ...
    message = 'The player appears to be in a game, but Roblox did not expose the server ID. “Try joining player” asks Roblox to join them and still respects their join settings.';
  } else {
    status = 'NO_PUBLIC_SERVER';
    message = 'Roblox did not expose a current game server. “Try joining player” lets Roblox check their live status and join permissions when the app opens.';
  }
